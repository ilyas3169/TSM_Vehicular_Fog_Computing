...
FinalDataX_in = 0; 
FinalDataY_in = 0; 
wait_in = randi([80 600],[Tks_in,1]);       % based on qualitative anlaysis after reading several published articles data...wait is in milliseconds 
DeviceCount_in = 0;
Inter_DA_in = 0;
Inter_SA_in = 0;
...
XCoordAboveBCQI = [(1:TKs_MonteCarlo)';300;0;1];
%YCoordUnder = [0 199 75 80 70 80 5 110 0 0];
% YCoordAbove = [10 200 12 185 6 192 15 197 12 197 42 165 170 10 195 15 196 14.61 137 220 220 10];
YCoordAbove = [TimeRsMonteCarlo;820;820;430];
YCoordAboveBCQI = [TimeRsMonteCarlo;820;820;430];
...
XCoordUnderBCQI = [(1:TKs_MonteCarlo)';300;0;1];
%YCoordUnder = [0 199 75 80 70 80 5 110 0 0];
% YCoordUnder = [10 200 12 185 6 192 15 197 12 197 42 165 170 10 195 15 196 14.61 137 0 0 10];
YCoordUnder = [TimeRsMonteCarlo;0;0;430];
YCoordUnderBCQI = [TimeRsMonteCarlo;0;0;430];
...
fill(XCoordUnderBCQI, YCoordUnderBCQI, c);
hold on;
%c = [0.4940 0.1840 0.5560];
c = [0.4660 0.6740 0.1880];
%fill(XCoordAbove,YCoordAbove,'red');
fill(XCoordAboveBCQI,YCoordAboveBCQI,'red');
  text(161,805,'Red Color: Region above curve','FontWeight','bold');
  text(161,765,'Blue Color: Area below curve','FontWeight','bold');
view(0,-90);
