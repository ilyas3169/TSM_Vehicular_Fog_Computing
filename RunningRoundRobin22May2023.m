...
%XCoordAbove = [(1:TKs_MonteCarlo)';223;0;0];
XCoordAbove = [(1:TKs_MonteCarlo)';300;0;1];
XCoordAboveRR = [(1:TKs_MonteCarlo)';300;0;1];
%YCoordUnder = [0 199 75 80 70 80 5 110 0 0];
% YCoordAbove = [10 200 12 185 6 192 15 197 12 197 42 165 170 10 195 15 196 14.61 137 220 220 10];
%YCoordAbove = [TimeRsMonteCarlo;1460;1460;852];
%YCoordAbove = [TimeRsMonteCarlo;1560;1560;1288];
YCoordAbove = [TimeRsMonteCarlo;1560;1560;812];

%XCoordUnder = [2 27 29 67 180 70 77 81 82 107 108 155 156 162 164 167 197 198 254 256 263 263 0 2];
%XCoordUnder = [(1:TKs_MonteCarlo)';223;0;0];
%XCoordUnder = [(1:TKs_MonteCarlo)';300;0;1];
XCoordUnderRR = [(1:TKs_MonteCarlo)';300;0;1];
%YCoordUnder = [0 199 75 80 70 80 5 110 0 0];
% YCoordUnder = [10 200 12 185 6 192 15 197 12 197 42 165 170 10 195 15 196 14.61 137 0 0 10];
%YCoordUnder = [TimeRsMonteCarlo;0;0;852];
% YCoordUnder = [TimeRsMonteCarlo;0;0;1288];
% YCoordUnder = [TimeRsMonteCarlo;0;0;812];
YCoordUnderRR = [TimeRsMonteCarlo;0;0;812];

%ax2 = axes(t);
c = [0.4940 0.1840 0.5560];    %purple color
c2 = [0.3010 0.7450 0.9330]; % bluesh
fill(XCoordUnderRR, YCoordUnderRR, c2);
hold on;
grayColor = [.7 .7 .7];
fill(XCoordAboveRR,YCoordAboveRR,grayColor);
 text(176,1510,'Gray Color: Region above curve','FontWeight','bold');
  text(176,1450,'Bluesh Color: Area below curve','FontWeight','bold');
view(0,-90);
